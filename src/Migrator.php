...
class Migrator {
	 ...
	 * @var string
	 */
	protected $table_name = 'database_migration_migrations';
	...
	protected function get_migrations( $exclude = array(), $migration = null ) {
		 ...
		 */
		$paths = apply_filters(
			'database_migration_migrations_paths',
			array(
	...
	protected function run_migration( $file, $name, $rollback = false ) {
		...
		$class_name    = Format::snake_to_pascal( $this->get_class_name( $name ) );
